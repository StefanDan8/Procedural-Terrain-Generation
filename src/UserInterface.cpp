...
parameters get_custom_parameters() {
   ...
   std::cout << "Chunk Size: " << params.chunkSize << ", chunksX: " << params.nChunksX << ", chunksY: " << params.nChunksY << std::endl;
...
parameters get_default_parameters() { return {16, 16, 16}; }
...
output_type get_output_method() {
   std::string input;
   std::cout << "Done! How would you like to save the output? (file/ppm/png): " << std::flush;

   std::cin >> input;
   std::string lower = toLower(input);
   int tries = 0;

   while (lower != "file" && lower != "ppm" && lower != "png" && tries < 3) {
        std::cout << "Invalid input. Please enter 'file', 'ppm' or 'png': " << std::flush;
        tries++;
   ...
   return (lower == "file") ? FILE : (lower == "ppm") ? PPM : PNG;
}

std::string get_file_path() {
   std::string input;
   std::cout << "And where would you like to save the file (include the filename please!): " << std::flush;
